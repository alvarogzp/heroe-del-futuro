...
class Level3 extends GameCanvas {
	private int score,sleepTime,cicle,lives,shield,indice_in,indice,xTiles,yTiles,deltaX,deltaY,finishLevel,cicloCount,cicloEspera,heroBullets,monsterLives,tiempoFin,volumenMusica,volumenEfectos,sleepTimeStatic,pLenght,scoreLevel2,ciclosDisparo,tiempoCreditos,heroDobleBullets,puntosBonus25,puntosBonus50,vidaBonus,superDisparos,superDisparoActivo,superDisparosTotales;
	private boolean playing,fireOn,cargandoActivo,tiempoFinOn,muerto,tecla,pausa,dobleDisparo,teclaIzquierda,teclaDerecha,teclaArriba,teclaAbajo,tiles,gameCpress,superDisparo,gameDpress,cargarSonido,comprobarPaint;
	...
	void cargaMusica() {
		...
		musica[1] = new Sonido("/3backmusic.wav",volumenMusica,cargarSonido);
		cargandoActualiza(11);
		musica[2] = new Sonido("/3monster.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(12);
		musica[3] = new Sonido("/explode0.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(13);
		musica[4] = new Sonido("/2explode12.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(14);
		musica[5] = new Sonido("/3explode2(2).wav",volumenEfectos,cargarSonido);
		cargandoActualiza(15);
		musica[6] = new Sonido("/3explodemonster.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(16);
		musica[7] = new Sonido("/herobullet.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(17);
		musica[8] = new Sonido("/2enemybullet1.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(18);
		musica[9] = new Sonido("/2monsterbullet.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(19);
		musica[10] = new Sonido("/2enemybullet3.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(20);
		musica[11] = new Sonido("/3explode1(2).wav",volumenEfectos,cargarSonido);
		cargandoActualiza(21);
		musica[12] = new Sonido("/pickweapon.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(22);
		musica[13] = new Sonido("/2superdisparo.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(23);
		musica[14] = new Sonido("/3dobledisparo.wav",volumenEfectos,cargarSonido);
		cargandoActualiza(24);
		musica[15] = new Sonido("/3finalmusic.wav",volumenEfectos,cargarSonido);
		musica[16] = new Sonido("/3live.wav",volumenEfectos,cargarSonido);
		musica[17] = new Sonido("/bonus25.wav",volumenEfectos,cargarSonido);
		musica[18] = new Sonido("/bonus50.wav",volumenEfectos,cargarSonido);
	...
	int volumen(String tipo,int accion) {
		int i;
		
		if (isPlaying()) {
			if (tipo == "musica") {
				musica[1].setVolumen(musica[1].getVolumen() + accion);
				volumenMusica=musica[1].getVolumen();
				return volumenMusica;
			} else {
				for (i=2; i<=pLenght; i++) {
					musica[i].setVolumen(musica[i].getVolumen() + accion);
				}
				volumenEfectos=musica[pLenght].getVolumen();
				return volumenEfectos;
			}
		} else {
			if (tipo == "musica") {
				if (volumenMusica+accion > 100 || volumenMusica+accion < 0) {
					return accion;
				} else {
					volumenMusica+=accion;
					return volumenMusica;
				}
			} else {
				if (volumenEfectos+accion > 100 || volumenEfectos+accion < 0) {
					return accion;
				} else {
					volumenEfectos+=accion;
					return volumenEfectos;
				}
			}
		}
